function [new_height_iceberg, new_length_iceberg, new_width_iceberg, volume_loss, mass_loss, height_loss] = Iceberg_Melt(vi, Tw, length_iceberg, width_iceberg,height_iceberg, va, vw, time_step)
...
height_loss = 0.58 * ((sqrt((vw(1) - vi(1))^2 + (vw(2) - vi(2))^2))^0.8) * (Tw - (-10)) * (abs(length_iceberg) ^ (-0.2));
